...
#define port_Send(dat)              SPI_Replace_Byte(3,dat) //spi 发送/接收 单个字节
#define port_delay_ms(ms)           soft_delay_ms(ms)       //ms延时
#define port_delay_us(us)           soft_delay_us(us)
//nrf需要的依赖初始化
...
{
    NVIC_DisableIRQ(NRF24L01_IQR_Channel);    //关闭对应外部中断
...
void nRF24L01_Restart(void)
{
    ...
    //NVIC_EnableIRQ(NRF24L01_IQR_Channel);
}
